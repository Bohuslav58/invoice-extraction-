...
def mistral_processsing(image_path, file_path_):
...
# Define the messages for the chat
    messages = [
            ...
            "content": [
                {
                "type": "text",
                "text": "extract data from invoice, and convert to json"
                },
            {
                "type": "image_url",
...
if 'Mistral' not in st.session_state:
    st.session_state['Mistral'] = True

if st.sidebar.checkbox('Processing by Mistral', value=st.session_state['Mistral']):
    st.session_state['Mistral'] = True
else:
    st.session_state['Mistral'] = False
st.write(st.session_state['Mistral'])
...
for i in files_path:
    ...
    print(st.session_state['Mistral'])
    if file_type.suffix == '.jpg' and st.session_state['Mistral'] == False:

        filling_mindee_xml(os.path.join(os.getcwd() + '/invoices/'+i),os.path.join(os.getcwd() + '/isdoc/'+new_file_name))
    else:
        new_file_name = os.path.join(str(base[0]) + ".json")
        print(new_file_name)
        mistral_processsing(os.path.join(os.getcwd() + '/invoices/'+i), os.path.join(os.getcwd() + '/json/'+new_file_name))
...
if "Invoice_correction" not in st.session_state:
        st.session_state.Invoice_correction = False

if st.sidebar.button('Invoice correction') or st.session_state.Invoice_correction:

    st.title('Invoice correction ' + i)

    #invoice_data=pd.DataFrame([dict])
    st.session_state.Invoice_correction = True
    correction=True
    ...
    with st.form("my_form"):
        ...
        if "df" not in st.session_state:
            st.session_state.df = pd.DataFrame([dict])
        st.session_state.df = st.data_editor(st.session_state.df)
        st.session_state.df.to_csv('data1.csv')
